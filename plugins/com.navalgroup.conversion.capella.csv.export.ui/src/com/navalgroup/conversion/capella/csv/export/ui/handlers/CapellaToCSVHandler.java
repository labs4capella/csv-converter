...
import com.navalgroup.conversion.capella.csv.export.CapellaToCSVJob;
...
/**
 * Capella to CSV Handler.
 * 
 * @author arichard
 *
 */
public class CapellaToCSVHandler extends AbstractCapellaConversionHandler {
	...
	protected void callDialog(Shell activeShell, Session openedSession, EObject rootObject) {
		...
		if (returnButton == IDialogConstants.OK_ID) {
			...
			CSVSettings csvSettings = new CSVSettings(exportDirectoryPath, fieldDelimiter, textDelimiter, characterSet,
					lineSeparator);
			CapellaToCSVJob job = new CapellaToCSVJob(rootObject, csvSettings);
