...
import com.navalgroup.conversion.capella.csv.core.jobs.GenerateEmptyCSVJob;
...
/**
 * Generate empty CSV Handler.
 * 
 * @author arichard
 *
 */
public class GenerateEmptyCSVHandler extends AbstractCapellaConversionHandler {
	...
	protected void callDialog(Shell activeShell) {
		...
		if (returnButton == IDialogConstants.OK_ID) {
			...
			CSVSettings csvSettings = new CSVSettings(exportDirectoryPath, fieldDelimiter, textDelimiter, characterSet,
					lineSeparator);
			GenerateEmptyCSVJob job = new GenerateEmptyCSVJob(csvSettings);
